...
def test_delete_icon_is_visible(page: Page):
    ...
    # The frontend is running on the 'frontend' service, accessible at its service name and port.
    page.goto("http://frontend:8444/login")
    page.get_by_label("Email").fill("test@example.com")
    page.get_by_label("Password").fill("password")
    page.get_by_role("button", name="Login").click()

    # Wait for navigation to the home page after login
    expect(page).to_have_url("http://frontend:8444/")

    # 2. Act: Navigate to the Tasks page.
    page.get_by_role("link", name="Tasks").click()
    expect(page).to_have_url("http://frontend:8444/tasks")
    ...
    page.screenshot(path="/app/jules-scratch/verification/tasks-page.png")