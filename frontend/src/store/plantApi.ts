...
import { Plant, GardenPlan, Planting, PlantingCreatePayload, Task, User, RecurringTask } from '../types';
...
export const plantApi = createApi({
  ...
  endpoints: (builder) => ({
    ...
    getPlants: builder.query<Plant[], void>({
      query: () => 'plants/',
      providesTags: (result) => result ? [...result.map(({ id }) => ({ type: 'Plant' as const, id })), { type: 'Plant', id: 'LIST' }] : [{ type: 'Plant', id: 'LIST' }],
    }),
    ...
    updatePlant: builder.mutation<Plant, Partial<Plant> & Pick<Plant, 'id'>>({
      query: ({ id, ...patch }) => ({ url: `plants/${id}`, method: 'PUT', body: patch }),
    ...
    importPlants: builder.mutation<{ message: string }, { file: File; mode: 'append' | 'replace' }>({
        query: ({ file, mode }) => {
            ...
            return { url: `plants/import?mode=${mode}`, method: 'POST', body: formData }
        },
        invalidatesTags: [{ type: 'Plant', id: 'LIST' }],
    }),
    ...
    getGardenPlans: builder.query<GardenPlan[], void>({
        query: () => 'garden-plans/',
        providesTags: (result) => result ? [...result.map(({ id }) => ({ type: 'GardenPlan' as const, id })), { type: 'GardenPlan', id: 'LIST' }] : [{ type: 'GardenPlan', id: 'LIST' }],
    }),
    ...
    getMostRecentGardenPlan: builder.query<GardenPlan, void>({
        ...
        providesTags: (result, error, id) => result ? [{ type: 'GardenPlan', id: result.id }] : [],
    }),
    addGardenPlan: builder.mutation<GardenPlan, Partial<GardenPlan>>({
        query: (body) => ({ url: 'garden-plans/', method: 'POST', body }),
        invalidatesTags: [{ type: 'GardenPlan', id: 'LIST' }],
    }),
    deleteGardenPlan: builder.mutation<{ success: boolean; id: number }, number>({
      query: (id) => ({ url: `garden-plans/${id}`, method: 'DELETE' }),
      invalidatesTags: (result, error, id) => [{ type: 'GardenPlan', id: 'LIST' }],
    }),
    touchGardenPlan: builder.mutation<GardenPlan, number>({
        query: (id) => ({ url: `garden-plans/${id}/touch`, method: 'PUT' }),
        invalidatesTags: (result, error, id) => [{ type: 'GardenPlan', id: 'LIST' }],
    }),
    ...
    addPlanting: builder.mutation<Planting, { planId: number, payload: PlantingCreatePayload }>({
        query: ({ planId, payload }) => ({ url: `garden-plans/${planId}/plantings`, method: 'POST', body: payload }),
    ...
    updatePlanting: builder.mutation<Planting, Partial<Planting> & Pick<Planting, 'id'>>({
        query: ({ id, ...patch }) => ({ url: `plantings/${id}`, method: 'PUT', body: patch }),
    ...
    deletePlanting: builder.mutation<{ success: boolean; id: number }, number>({
        query(id) { return { url: `plantings/${id}`, method: 'DELETE' } },
    ...
    updateTask: builder.mutation<Task, Partial<Task> & Pick<Task, 'id'>>({
        ...
        invalidatesTags: (result, error, { id }) => [{ type: 'Task', id }, { type: 'Task', id: 'LIST' }, { type: 'GardenPlan', id: result?.garden_plan_id }],
    }),
    deleteTask: builder.mutation<{ success: boolean; id: number }, number>({
        query(id) { return { url: `tasks/${id}`, method: 'DELETE' } },
    ...
    updateTaskGroup: builder.mutation<Task[], { groupId: number, dateDiffDays: number }>({
      query: ({ groupId, dateDiffDays }) => ({ url: `task-groups/${groupId}`, method: 'PUT', body: { date_diff_days: dateDiffDays } }),
      invalidatesTags: [{ type: 'Task', id: 'LIST' }, { type: 'GardenPlan', id: 'LIST' }],
    }),
    unlinkTaskGroup: builder.mutation<Task[], { groupId: number }>({
      query: ({ groupId }) => ({ url: `task-groups/${groupId}/unlink`, method: 'PUT' }),
      invalidatesTags: [{ type: 'Task', id: 'LIST' }, { type: 'GardenPlan', id: 'LIST' }],
    }),

    // --- Misc Endpoints ---
    importMappedPlants: builder.mutation<{ message: string }, { data: any[], mapping: Record<string, string> }>({
        query: (body) => ({ url: 'plants/import-mapped', method: 'POST', body }),
        invalidatesTags: [{ type: 'Plant', id: 'LIST' }],
    }),
